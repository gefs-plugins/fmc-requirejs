...
define(['distance', 'flight', 'math'], function (distance, flight, math) {
	...
	return {
		...
		update: function (waypoints) {
			...
			this.print(flightdist, nextdist, times);
			this.printNextWaypointInfo(waypoints, nextWaypoint, lat1, lon1);
		},
		 ...
		 */
		updatePhase: function (waypoints) {
		 ...
		 */
		print: function (flightdist, nextdist, times) {
		...
		printNextWaypointInfo: function (waypoints, n, lat, lon) {
			...
			if (arguments.length !== 4) { // Function called by `change` in jQuery events or this.shiftWaypoint (no current coords)
				...
				if (n) { // If there is a next waypoint as defined in this.nextWaypoint
